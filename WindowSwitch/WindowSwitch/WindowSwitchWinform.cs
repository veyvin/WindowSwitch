...
{
    public class WindowSwitchWinform:IWindowSwitchCtrl
    {
        ...
        /// <summary>
        ///一、第一个参数传入-parentHWND时，第二个参数传入父级节点，其他参数
        ///二、 第一个参数传入是否隐藏父级，第二个参数传入父级节点，其他参数
        /// </summary>
        /// <param name="argus"></param>
        /// <returns></returns>
        public bool OnOpenedByParent(ref string[] argus)
        {
            if (argus == null || argus.Length == 0) return false;
            loadAsChild = argus[0] == "-parentHWND";
            if (!loadAsChild){
                hideParent = new IntPtr(int.Parse(argus[0])) == (IntPtr)0;
            }

            ///是否隐藏了父级
            if (argus.Length > 1) {
                parentWindow = new IntPtr(int.Parse(argus[1]));
            }
            ///用户自定义参数
            if (argus.Length > 2)
            {
